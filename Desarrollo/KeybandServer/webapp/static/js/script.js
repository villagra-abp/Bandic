...
app.controller("ControllerProducto", function($scope, $http, Upload) {
	...
	$scope.addProducto = function() {
       ...
       //put producto
		...
		var enviar = {"id":idrandom,"nombre": $scope.nuevoProducto,"descripcion":"un maravilloso producto del hotel",
				"precio":2.50,"iva":21,"tweet":"Disfrutando de este maravilloso producto en el mejor hotel",
...
app.controller("ControllerTicket", function($scope,$http) {
	...
	function crearFactura(response, response2) {
		var fecha = response[0].fecha.split(" ");
    	var fecha1 = fecha[0].split("-");
    	var fecha2 = fecha[1].split(":");
    	var total = 0;

		for(var i=0; i<response2.data.length; i++) {
			total = total + parseInt(response2.data[i].importe);
		}
		
		////// AQUI HABRIA QUE PREGUNTARSE POR EL ROL DE USUARIO (POR SI LE SALE GRATIS AL SER TODO INCLUIDO POR EJ)////////
    	...
    	var lines = response2.data.length;
    	for(var j=0; j<response2.data.length; j++) {
    		dd.content[5].table.body[j+1] = [response2.data[j].cantidad.toString(),response2.data[j].nombre, response2.data[j].precio.toString(), response2.data[j].importe.toString()];
    	}
